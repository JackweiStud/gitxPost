...
范围：`web/ui/src/views/PublishPage.vue`、`web/api/server.py`

### 问题

- `/publish` 将 `359/280` 视为超限，禁用发布按钮并标红。
- API `/api/publish/post` 对正文强制 280 字符上限，与「只计数、不卡发」的预期不一致。

### 变更

- 前端：有非空正文即可发布；计数文案改为「N 字」；去掉超限样式；圆环仍以 280 为满环参考，超长时保持满环且不用红色「错误」色。
- 后端：移除 280 字符校验；实际长度仍由 `xpost` / `auto_publish_post`（如 `MAX_TEXT_LENGTH`）在发布流程中约束。

### 验证

- 未运行完整 UI/E2E；改动为局部逻辑与 API 校验删除，建议本地打开发布页确认按钮与请求行为。

### 风险

- 极长正文可能在浏览器自动化或 X 侧失败，需依赖既有 xpost 错误提示。
...
- 原逻辑按"本地时间当天"过滤，存在时区混用 bug，且早上运行无法覆盖完整 24 小时周期。

### 变更

- 改为「过去 24 小时」滑动窗口：从当前 UTC 时间往前推 24 小时，保留该范围内的推文。
- 日志/文件命名保持不变，仍用本地时间日期（如 `2026-04-16.md`）。

### 验证

- 北京时间 4/16 09:00 运行 → 过滤范围 GMT 4/15 01:00 ~ 4/16 01:00（完整 24h）
- 不再受本地时区影响，全球推文覆盖完整。

### 风险

- 无，滑动窗口逻辑更稳定，适合全球数据采集场景。
